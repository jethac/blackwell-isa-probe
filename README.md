...
- **`.multicast::cluster` TMA: accepted ≠ implemented.** It assembles on consumer *with a ptxas advisory warning* and lowers to a plain **unicast** `UTMALDG.2D` — byte-identical to the non-multicast form, versus real multicast on B200. ([`probes/j_multicast_tma.cu`](probes/j_multicast_tma.cu).)
